...
neighborhood_clause = "AND Neighborhood IN ('SOUTH LAKE UNION', 'BELLTOWN')"
...
bbox = slu_denny_triangle_bbox
ll_neighborhood_clause = f"AND Latitude >= {bbox[0]} AND Latitude <= {bbox[1]} AND Longitude >= {bbox[2]} AND Longitude <= {bbox[3]}"
result = conn.execute(
    f"""
